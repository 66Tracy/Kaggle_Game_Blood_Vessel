...
def single_image_iou(mask, gt):
    ...
    rle2 = mask_utils.encode(np.asfortranarray(gt.astype(np.uint8)))
...
@torch.inference_mode()
def eval_miou(model, data_loader, device):
    model.eval()
    iou = 0
    for images, targets in data_loader:
        ...
        for m in range(len(pred[0]['masks'])):
            ...
            # 得到一系列mask
            if pred[0]['scores'][m].detach().cpu().numpy()>=0.5:
                img_mask += mask.astype(int)
        
        # (n, 512, 512)
        gts = targets[0]["masks"].detach().cpu().numpy()
        ...
        iou += single_image_iou(img_mask, img_gt)

    return iou/len(data_loader)